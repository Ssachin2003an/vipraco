...
export default function Chat() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'bot',
      text: `Hey ${user.first_name}! I'm VipraCo, your HR assistant for ${user.org_name}. Ask me anything about your leave, salary, profile, or company policies.`
    }
  ]);
  ...
  const send = async (text) => {
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        { role: 'bot', text: err?.response?.data?.error || 'Something went wrong. Please try again.' }
      ]);
    } finally {
      setBusy(false);
    }
    ...
    <div className="h-screen flex bg-gray-100">
      ...
      <aside className="w-72 bg-white border-r border-gray-200 p-5 hidden md:flex md:flex-col">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="font-bold text-gray-900">VipraCo</span>
        </div>
      ...
      <main className="flex-1 flex flex-col">
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-gray-900">VipraCo Assistant</h2>
        ...
        >
          <div className="max-w-2xl mx-auto flex gap-2">
            <input
              ...
              placeholder="Ask VipraCo anything about HR..."
